...
for(i in 1:length(files.v)){
  ...
  chunk.data.l <- getSwordSegmentTableList(doc.object, 30)
...
results.t <-table(pred.svm, class.f)
str(results.t)
results.t[, 1]
sum(results.t["Aesch", ])

table(pred.svm, class.f)
write.table(pred.svm, file="results/clusters/test1.csv")

test.results = NULL
test.results <-append(test.results, pred.svm)
test.results
