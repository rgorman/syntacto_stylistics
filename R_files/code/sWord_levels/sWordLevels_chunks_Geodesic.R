...
input.dir <- "../sWord_levels/files_with_sWords/geodesicOnly"
...
write.csv(ordered.df, file = "Rresults/matrices/sWordLevels_1000tokenChunks_June_11-2016.csv")




swords <-getNodeSet(doc.object, "//word/*")
swordLevels <- getNodeSet(doc.object, "//word/*")
geodesics <- getNodeSet(doc.object, "//word/*[last()]", )
swords
sword.content <- paste(sapply(swords, xmlValue), collapse=NULL)
swordLevels.content <- paste(sapply(swordLevels, xmlValue), collapse=NULL)
geodesic.content <- paste(sapply(geodesics, xmlValue), collapse=NULL)
sword.content[1:10]
geodesic.content[1:10]
